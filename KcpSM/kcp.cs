...
public class KCP
{
    ...
    //调用Send以后放入snd_queue(分好片)
    Segment[] snd_queue = new Segment[0];
    //接收数据后待被上层取走的队列,并且本地wnd也是这个queue的容量,rcv_buf收到数据以后经过check以后进入这里
    Segment[] rcv_queue = new Segment[0];
    //flush的时候远端cwnd足够的话,讲snd_queue中的segment移到这里
    Segment[] snd_buf = new Segment[0];
    //接收数据缓冲区(调用input后进入这里)
    ...
    void parse_data(Segment newseg) {
        ...
        if (!repeat) {
            //不重复但是idx是-1表示新收到的数据包在rev_buf的最前面,将新收到的数据包插入到rcv_buf
            if (after_idx == -1)
        ...
        // move available data from rcv_buf -> rcv_queue
        var count = 0;
        //校验rcv_buf中的数据,成功的话讲它移入rcv_queue,等待上层recv()走
        foreach (var seg in rcv_buf) {
            if (seg.sn == rcv_nxt && rcv_queue.Length < rcv_wnd)
            {
                rcv_queue = append<Segment>(rcv_queue, seg);
    ...
    public int Input(byte[] data) {
        ...
        while (true)
        {
            ...
            //更新远端WND值
            rmt_wnd = (UInt32)wnd;
            //解析本地send_buf的左边界,删除小于该数值的send_buf中的数据(因为已确认)
            parse_una(una);
            //更新本地send_una值(该值为本地未确认的send_buf中的最小值,若send_buf为空,则是send_nxt)
            shrink_buf();
            //是一个ack数据包
            if (IKCP_CMD_ACK == cmd) {
            ...
            //是一个push数据包
                ...
                if (_itimediff(sn, rcv_nxt + rcv_wnd) < 0) {
                    ...
                    if (_itimediff(sn, rcv_nxt) >= 0) {
                        ...
                        //插入rcv_buf,并且更新rcv_nxt(有可能变更了),校验rcv_buf到rcv_queue,收缩插入rcv_buf,具体放在函数内注释
                        parse_data(seg);
                    }
                }
            }
